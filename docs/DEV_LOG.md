...
3. ~~No CI run has happened.~~ **Resolved 2026-07-30 on the first pipeline run.**
   `Backend CI` and `Frontend CI` both passed first time. `OpenAPI Lint` failed
   once: `bash -o globstar` is invalid, because `globstar` is a `shopt` option
   rather than a `set -o` one. Fixed by enabling it inside the script with
   `shopt -s globstar nullglob`. Worth remembering that `nullglob` matters as much
   as `globstar` there — an unmatched `**` pattern otherwise expands to the
   literal string, the array looks non-empty, and the "no specs yet" guard never
   fires.
4. ~~The integration suite has never run against real PostgreSQL.~~ **Resolved
   2026-07-30:** 5 integration tests passed against the postgres:16 service,
   after `alembic upgrade head` and a `downgrade base && upgrade head`
   reversibility check. The SQLite/PostgreSQL split (D-08) is therefore proven end
   to end, and the `assert not settings.is_sqlite` guard confirmed the suite
   really was on PostgreSQL rather than silently falling back.
